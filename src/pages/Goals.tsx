...
import { Target, CheckSquare, Calendar as CalendarIcon, PlusCircle, Trash2, Star, Trophy } from "lucide-react";
            ...
            {sortedGoals.length > 0 ? (
              <div className="space-y-4">
                {sortedGoals.map(goal => (
                  <Card key={goal.id} className={`p-4 ${goal.isPinned ? 'border-yellow-400 dark:border-yellow-600 border-2' : ''} hover:shadow-md transition-shadow`}>
                        ...
                        <details>
                          ...
                          <div className="mt-2 space-y-2">
                            {goal.steps.map(step => (
                              <div 
                                key={step.id} 
                                className="flex items-center space-x-2 p-2 bg-gray-50 dark:bg-gray-700/50 rounded"
                                onClick={() => handleToggleStep(goal.id, step.id)}
                              >
                                <CheckSquare className={`h-4 w-4 cursor-pointer ${step.isCompleted ? 'text-green-500 fill-green-500' : 'text-gray-400'}`} />
