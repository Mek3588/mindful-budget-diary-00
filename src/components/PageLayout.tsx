...
interface PageLayoutProps {
  ...
  title: string;
  icon: ReactNode;
  pageType: 'diary' | 'budget' | 'notes' | 'calendar' | 'security' | 'settings' | 'goals' | 'medical';
}
...
export function PageLayout({ children, title, icon, pageType }: PageLayoutProps) {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-${gradientFrom} to-${gradientTo}`}
      ...
      <nav className="fixed top-0 left-0 right-0 z-30 bg-white/80 backdrop-blur-lg border-b border-gray-200 dark:bg-gray-900/80 dark:border-gray-700">
